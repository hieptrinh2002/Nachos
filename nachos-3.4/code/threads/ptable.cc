...
int PTable::JoinUpdate(int id)
{
    ...
    {
        printf("\nPTable::JoinUpdate: Can't join in process that is not its parent process.\n");
        return -1;
    }
...
int PTable::ExitUpdate(int exitcode)
{
    ...
    if (id == 0)
    {
        currentThread->FreeSpace(); // Giai phong vung nho cho thread
        interrupt->Halt();          // Tat chuong trinh
        return 0;
    }

    // Kiem tra xem processID nay co ton tai khong
    if (IsExist(id) == false)
    {
    ...
    pcb[pcb[id]->parentID]->DecNumWait(); // Giam thoi gian cho cua thang cha
    ...
    // Xoa process nay nay khong ptable
    Remove(id);
    return exitcode;
}
